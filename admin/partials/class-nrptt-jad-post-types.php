...
class NRPTT_JAD_Post_types {
    ...
    public static function init() {
        ...
        add_action('init', array(__CLASS__, 'nrptt_jad_register_taxonomies'), 5);
        add_action('init', array(__CLASS__, 'nrptt_jad_register_post_types'), 5);
        add_action('init', array(__CLASS__, 'nrptt_jad_register_post_status'), 10);
        add_action('restrict_manage_posts', array(__CLASS__, 'nrptt_jad_ipn_filter'), 10);
        add_action('add_meta_boxes', array(__CLASS__, 'nrptt_jad_remove_meta_boxes'), 10);
        add_action('manage_edit-paypal_ipn_columns', array(__CLASS__, 'nrptt_jad_add_paypal_ipn_columns'), 10, 2);
        add_action('manage_paypal_ipn_posts_custom_column', array(__CLASS__, 'nrptt_jad_render_paypal_ipn_columns'), 2);
        add_filter('manage_edit-paypal_ipn_sortable_columns', array(__CLASS__, 'nrptt_jad_paypal_ipn_sortable_columns'));
        add_action('pre_get_posts', array(__CLASS__, 'nrptt_jad_ipn_column_orderby'));
        add_filter('views_edit-paypal_ipn', array(__CLASS__, 'nrptt_jad_ipn_section_name'), 10, 1);
        add_action('add_meta_boxes', array(__CLASS__, 'nrptt_jad_add_meta_boxes_ipn_data_custome_fields'), 31);
        add_filter('nrptt_jad_the_meta_key', array(__CLASS__, 'nrptt_jad_the_meta_key_remove_raw_dump'), 10, 3);
        add_action('add_meta_boxes', array(__CLASS__, 'nrptt_jad_add_meta_boxes_ipn_data_serialized'), 31);
        add_action('add_meta_boxes', array(__CLASS__, 'nrptt_jad_add_meta_boxes_provide_hook_function_snippets'), 31);
        add_filter('post_class', array(__CLASS__, 'nrptt_jad_post_class_representation'), 10, 3);
        add_action('parse_query', array(__CLASS__, 'nrptt_jad_parse_query'), 10, 1);
    }

    /**
     * nrptt_jad_register_taxonomies function.
     *
     * @since    1.0.0
     * @access   public
     */
    public static function nrptt_jad_register_taxonomies() {

        if (taxonomy_exists('paypal_ipn_type')) {
            return;
        }

        do_action('nrptt_jad_register_taxonomy');

        register_taxonomy('paypal_ipn_type', apply_filters('nrptt-jad_taxonomy_objects_ipn_cat', array('paypal_ipn')), apply_filters('nrptt-jad_taxonomy_args_ipn_cat', array(
            'hierarchical' => true,
    ...
    /**
     * nrptt_jad_register_post_types function
     * @since    1.0.0
     * @access   public
     */
    public static function nrptt_jad_register_post_types() {
        global $wpdb;
        if (post_type_exists('paypal_ipn')) {
            return;
        }

        do_action('nrptt_jad_register_post_type');

        register_post_type('paypal_ipn', apply_filters('nrptt_jad_register_post_type_ipn', array(
            'labels' => array(
            ...
            'rewrite' => array('slug' => 'paypal_ipn'),
            'query_var' => true,
            'menu_icon' => PIW_PLUGIN_URL . 'admin/images/nrptt-jad-icon.png',
            'supports' => array('', ''),
            'has_archive' => true,
     ...
     * @access   public
     */
    public static function nrptt_jad_register_post_status() {
        global $wpdb;

        $ipn_post_status_list = self::nrptt_jad_get_ipn_status();
    ...
    /**
     * nrptt_jad_ipn_filter function  used for IPN status and type that will display admin side.
     * @since    1.0.0
     * @access   public
     */
    public static function nrptt_jad_ipn_filter() {
    ...
    /**
     * nrptt_jad_remove_meta_boxes function used for remove submitdiv meta_box for paypal_ipn custome post type
     ...
     * @access   public
     */
    public static function nrptt_jad_remove_meta_boxes() {
    ...
    /**
     * nrptt_jad_get_ipn_status helper function used for return IPN status
     * @since    1.0.0
     * @access   public
     */
    public static function nrptt_jad_get_ipn_status() {
        global $wpdb;
    ...
    /**
     * nrptt_jad_get_ipn_status helper function used for return IPN status for filter
     * @since    1.0.0
     * @access   public
     */
    public static function nrptt_jad_get_ipn_status_filter() {
        global $wpdb;
     ...
     * @return array
     */
    public static function nrptt_jad_add_paypal_ipn_columns($existing_columns) {
        $columns = array();
    ...
    /**
     * nrptt_jad_render_paypal_ipn_columns helper function used add own column in IPN listing
     * @since    1.0.0
     * @access   public
     */
    public static function nrptt_jad_render_paypal_ipn_columns($column) {
        global $post;
        ...
        switch ($column) {
            ...
            case 'payment_date' :
                ...
                if (isset($payment_date) && !empty($payment_date)) {
                    self::nrptt_jad_date_parsing($payment_date);
                } else {
			...
			self::nrptt_jad_date_parsing($subscr_date);
                    }
                }
                break;
            case 'first_name' :
    ...
    /**
     * nrptt_jad_paypal_ipn_sortable_columns helper function used for make column shortable.
     * @since    1.0.0
     * @access   public
     * @return $columns
     */
    public static function nrptt_jad_paypal_ipn_sortable_columns($columns) {
    ...
    /**
     * nrptt_jad_ipn_column_orderby helper function used for shorting query handler
     * @since    1.0.0
     * @access   public
     */
    public static function nrptt_jad_ipn_column_orderby($query) {
        global $wpdb;
    ...
    /**
     * nrptt_jad_ipn_section_name helper function used for Make section name when it upto 20 characters.
     * @since    1.0.0
     * @access   public
     * @return array
     */
    public static function nrptt_jad_ipn_section_name($sectionlist) {
    ...
    /**
     * nrptt_jad_add_meta_boxes_ipn_data_custome_fields function used for register own meta_box for display IPN custome filed read only
     * @since    1.0.0
     */
    public static function nrptt_jad_add_meta_boxes_ipn_data_custome_fields() {

        add_meta_box('paypal-ipn-ipn-data-custome-field', __('PayPal IPN Fields', 'paypal-ipn'), array(__CLASS__, 'nrptt_jad_display_ipn_custome_fields'), 'paypal_ipn', 'normal', 'high');
    }

    /**
     * nrptt_jad_display_ipn_custome_fields helper function used for display raw dump in html format
     * @since    1.0.0
     * @access   public
     */
    public static function nrptt_jad_display_ipn_custome_fields() {
        if ($keys = get_post_custom_keys()) {
            ...
            foreach ((array) $keys as $key) {
                 ...
                 */
                echo apply_filters('nrptt_jad_the_meta_key', "<tr><th class='post-meta-key'>$key:</th> <td>$value</td></tr>", $key, $value);
            }
            echo "</table>";
            echo "</div>";
        }
    ...
    /**
     * nrptt_jad_the_meta_key_remove_raw_dump helper function used ignore specific key that will not display in raw dump
     * @since    1.0.0
     * @access   public
     */
    public static function nrptt_jad_the_meta_key_remove_raw_dump($row, $key, $value) {
    ...
    /**
     * nrptt_jad_add_meta_boxes_ipn_data_serialized function used for register own meta_box for display IPN row data custome post type
     * @since    1.0.0
     * @access   public
     */
    public static function nrptt_jad_add_meta_boxes_ipn_data_serialized() {

        add_meta_box('nrptt-jad-ipn-data-serialized', __('PayPal IPN Raw Data', 'paypal-ipn'), array(__CLASS__, 'nrptt_jad_add_meta_boxes_ipn_data_box'), 'paypal_ipn', 'advanced', 'high');
    }

    /**
     * nrptt_jad_add_meta_boxes_provide_hook_function_snippets function used for register own meta_box for display Provide hook function snippets in the IPN detail page
     * @since    1.0.0
     * @access   public
     */
    public static function nrptt_jad_add_meta_boxes_provide_hook_function_snippets() {

        add_meta_box('nrptt-jad-ipn-function', __('Hook Function Snippet', 'paypal-ipn'), array(__CLASS__, 'nrptt_jad_add_meta_boxes_hook_function_snippets'), 'paypal_ipn', 'advanced', 'high');
    }

    /**
     * nrptt_jad_add_meta_boxes_ipn_data_box helper function used for display IPN row data to bottom of detail section
     * @since    1.0.0
     * @access   public
     */
    public static function nrptt_jad_add_meta_boxes_ipn_data_box() {
        global $post;
        $post_id = $post->ID;
        echo '<pre />';
    ...
    /**
     * nrptt_jad_add_meta_boxes_hook_function_snippets helper function used for display IPN row data to bottom of detail section
     * @since    1.0.0
     * @access   public
     */
    public static function nrptt_jad_add_meta_boxes_hook_function_snippets() {
        global $post;
        $post_id = $post->ID;
        ...
        <div>
            ...
            <p class="content_padding"><?php _e('PayPal IPN for WordPress provides a <a target="_blank" href="https://www.angelleye.com/paypal-ipn-wordpress-hooks/?utm_source=nrptt_jad&utm_medium=docs_link_ipn_details&utm_campaign=nrptt_jad">wide variety of developer hooks</a> that you can use within your theme or plugins to trigger your own function(s).  There are hooks available based on the IPN type as well as the payment status of a transaction.', 'paypal-ipn'); ?></p>
    ...
    /**
     * nrptt_jad_post_class_representation helper function used for IPN listing highlight when is invalid
     * @since    1.0.0
     * @access   public
     */
    public static function nrptt_jad_post_class_representation($classes, $class, $postid) {
        global $post;
     ...
     * @access   public
     */
    public static function nrptt_jad_parse_query($query) {
    ...
    public static function nrptt_jad_date_parsing($date){
